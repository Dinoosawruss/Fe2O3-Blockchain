...
impl Blockchain { 
    ...
    pub fn update_with_block (&mut self, block: Block) -> Result<(), BlockValidationErr> {
        ...
        if block.index != i as u32 { return Err(BlockValidationErr::MismatchedIndex) }
                                                            // This value should be checked but ignored for simplicity
        else if !block::check_difficulty(&block.hash(), block.difficulty) { return Err(BlockValidationErr::InvalidHash); } 
            ...
            else if block.prev_block_hash != prev_block.hash() { return Err(BlockValidationErr::MismatchedPreviousHash); }
        } 
